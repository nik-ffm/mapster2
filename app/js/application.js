...
PouchNotesObj.prototype.savenote = function () {
    ...
    */
    ...
    o.cat      = (this.formobject.cat.value == '') ? '' : this.formobject.cat.value;
...
PouchNotesObj.prototype.search = function(searchkey) {
	...
	var map = function(doc) {
		...
		*/
		...
		if( regex.test(doc.notetitle) || regex.test(doc.note) || regex.test(doc.tags) || regex.test(doc.cat) ){		
...
var defaultCoords = [49.01, 8.41]
var map = L.mapbox.map('myPostsMap', 'mapbox.streets')
.setView(defaultCoords,13);
...
function goOfflineState(){
    $('#offlineNotification').show();
}
function goOnlineState(){
